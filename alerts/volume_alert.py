...
def calculate_volume_metrics(df):
    ...
    - volume_24h: сума за останні 24 бари (поточна година + 23 попередні)
    - volume_avg_14d: середнє за 14 днів (беремо volume_24h для кожного дня)
    
    ✅ ВИПРАВЛЕНО: правильний розрахунок середнього за 14 днів
    """
    if len(df) < 24:
        return None
    
    # Поточний об'єм за 24 години (останні 24 бари)
    volume_24h_current = df["volume_usdt"].tail(24).sum()
    
    # Розраховуємо volume_24h для кожного з останніх 14 днів
    volumes_14d = []
    
    for day in range(14):
        start_idx = -(24 + day * 24)  # Зсув назад: -24, -48, -72, ...
        ...
        # Беремо 24 бари для цього дня
        if end_idx:
            day_volume_24h = df["volume_usdt"].iloc[start_idx:end_idx].sum()
        else:
            day_volume_24h = df["volume_usdt"].iloc[start_idx:].sum()
        
        volumes_14d.append(day_volume_24h)
    
    # Якщо менше 14 днів даних - беремо що є
    if len(volumes_14d) == 0:
        return None
    
    # Середнє за 14 днів
    volume_avg_14d = sum(volumes_14d) / len(volumes_14d)
    
    return {
        "volume_24h": volume_24h_current,
        "volume_avg_14d": volume_avg_14d,
        "ratio": volume_24h_current / volume_avg_14d if volume_avg_14d > 0 else 0
    }
...
def check_volume_threshold(volume_avg_14d, ratio):
    ...
    ✅ ВИПРАВЛЕНО: правильна логіка порівняння
    """
    for max_avg, min_ratio in VOLUME_THRESHOLDS:
        if volume_avg_14d >= max_avg:
            if ratio >= min_ratio:
                return True
            # Якщо не пройшов поріг для цього діапазону - перевіряємо наступний
            continue
